...
data_set = '1'
path = r"data/YC0{}_rel.csv".format(data_set)

data = np.loadtxt(fname=path, skiprows=1, delimiter=",")[:, 3:]
...
if method == 'SVR':
    ...
    total[total < 0] = 0
    rmse, mae, acc, r2, var = evaluation(testY1, result1)
    print('SVR_rmse:%r' % rmse,
          'SVR_mae:%r' % mae,
          'SVR_acc:%r' % acc,
          'SVR_r2:%r' % r2,
          'SVR_var:%r' % var)
    rmse_ts, mae_ts, acc_ts, r2_ts, var_ts = rmse, mae, acc, r2, var
...
pure_data_file = r"D:\projects\SF_baselines\T-GCN-master\saved_model\pure_result.csv"
with open(pure_data_file, mode='a') as fin:
    str = "\n{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{},{},{},{}".format(rmse_ts, mae_ts, acc_ts, r2_ts, var_ts,
                                                            time_stamp, hyper_parameters, data_set, result_type)
    fin.write(str)
